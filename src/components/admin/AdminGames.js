...
const CATEGORIES = [
  'Battle Royale', 'MMO', 'MMORPG', 'ARPG', 'Extraction', 'Hero Shooter', 'RPG', 'FPS', 'Strategy', 'Sports',
]
...
export default function AdminGames({ secret }) {
    ...
    const res = await fetch('/api/admin', {
      ...
      body: JSON.stringify({
        ...
        data: {
          ...
          serviceCategories: editGameForm.serviceCategories || [],
        }
      }),
    })
    const d = await res.json()
  ...
  async function addService(gameId) {
    ...
    if (serviceForm.pricingType === 'quantity') {
      options = {
        type: 'quantity',
        unitName: serviceForm.pricingOptions.unitName,
        unitPrice: parseFloat(serviceForm.pricingOptions.unitPrice),
        minQty: parseInt(serviceForm.pricingOptions.minQty),
        maxQty: parseInt(serviceForm.pricingOptions.maxQty),
      }
    } else if (serviceForm.pricingType === 'range') {
      options = {
        type: 'range',
        unitName: serviceForm.pricingOptions.unitName,
        pricePerUnit: parseFloat(serviceForm.pricingOptions.pricePerUnit),
        min: parseInt(serviceForm.pricingOptions.min),
        max: parseInt(serviceForm.pricingOptions.max),
      }
    } else if (serviceForm.pricingType === 'options') {
      options = {
        type: 'options',
        choices: serviceForm.pricingOptions.choices,
      }
    }
  ...
  async function saveEditService() {
    console.log('pricingType:', editForm.pricingType)
    console.log('pricingOptions:', editForm.pricingOptions)
    let options = null
    if (editForm.pricingType === 'quantity') {
      options = {
        type: 'quantity',
        unitName: editForm.pricingOptions.unitName,
        unitPrice: parseFloat(editForm.pricingOptions.unitPrice),
        minQty: parseInt(editForm.pricingOptions.minQty),
        maxQty: parseInt(editForm.pricingOptions.maxQty),
      }
    } else if (editForm.pricingType === 'range') {
      options = {
        type: 'range',
        unitName: editForm.pricingOptions.unitName,
        pricePerUnit: parseFloat(editForm.pricingOptions.pricePerUnit),
        min: parseInt(editForm.pricingOptions.min),
        max: parseInt(editForm.pricingOptions.max),
      }
    } else if (editForm.pricingType === 'options') {
      options = {
        type: 'options',
        choices: editForm.pricingOptions.choices,
      }
    }
...
function SortableGameRow({
  game, editGame, editGameForm, setEditGame, setEditGameForm,
  showAddService, setShowAddService, editService, setEditService,
  editForm, setEditForm, serviceForm, setServiceForm,
      ...
      <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', overflow: 'hidden' }}>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            <div {...attributes} {...listeners} style={{ cursor: 'grab', color: 'var(--text-dim)', fontSize: '18px', padding: '0 4px', userSelect: 'none' }}>⠿</div>
            ...
            {!game.isActive && (
              <span style={{ background: '#2a1a1a', border: '1px solid #4a2a2a', borderRadius: '20px', padding: '2px 10px', fontSize: '11px', color: '#ff6666' }}>Pasif</span>
            )}
          </div>
          <div style={{ display: 'flex', gap: '8px' }}>
            <button className="btn-secondary" style={{ fontSize: '12px', padding: '6px 12px' }}
              onClick={() => {
                ...
                setEditGameForm({
                  ...
                  serviceCategories: game.serviceCategories || [],
                })
              }}>
        ...
        {editGame === game.id && (
          <div style={{ padding: '20px', borderBottom: '1px solid var(--border)', background: 'var(--bg-elevated)' }}>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '8px' }}>
                {(editGameForm.serviceCategories || []).map((cat, i) => (
                  <span key={i} style={{
                    ...
                    {cat}
                    <button type="button" onClick={() => setEditGameForm(f => ({
                      ...f, serviceCategories: f.serviceCategories.filter((_, j) => j !== i)
                    }))} style={{ background: 'none', border: 'none', color: 'var(--gold)', cursor: 'pointer', fontSize: '14px', lineHeight: 1, padding: 0 }}>×</button>
                  </span>
                ))}
              </div>
              <div style={{ display: 'flex', gap: '8px' }}>
                <input
                  ref={catInputRef}
                  type="text"
                  placeholder="Yeni hizmet kategorisi..."
                  onKeyDown={e => {
                    if (e.key === 'Enter' && e.target.value.trim()) {
                      setEditGameForm(f => ({
                        ...f,
                        serviceCategories: [...(f.serviceCategories || []), e.target.value.trim()]
                      }))
                      e.target.value = ''
                    }
                  }}
                  style={{
                    ...
                    borderRadius: '6px', padding: '7px 10px', color: '#fff', fontSize: '13px',
                    fontFamily: 'var(--font-inter)', outline: 'none',
                  }}
                />
                <button type="button" className="btn-secondary" style={{ fontSize: '12px', padding: '6px 14px' }}
                  onClick={() => {
                    const val = catInputRef.current?.value?.trim()
                    console.log('input val:', val)
                    console.log('mevcut kategoriler:', editGameForm.serviceCategories)
                    if (val) {
                      setEditGameForm(f => {
                        console.log('yeni kategoriler:', [...(f.serviceCategories || []), val])
                        return { ...f, serviceCategories: [...(f.serviceCategories || []), val] }
                      })
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <textarea value={editGameForm.description} onChange={e => setEditGameForm(f => ({ ...f, description: e.target.value }))}
                rows={3} placeholder="Oyun hakkında kısa açıklama..."
                style={{ width: '100%', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '6px', padding: '8px 10px', color: '#fff', fontSize: '13px', fontFamily: 'var(--font-inter)', outline: 'none', resize: 'vertical' }} />
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button className="btn-primary" style={{ fontSize: '13px', padding: '8px 16px' }} onClick={saveEditGame}>Kaydet</button>
              <button className="btn-secondary" style={{ fontSize: '13px', padding: '8px 16px' }} onClick={() => setEditGame(null)}>İptal</button>
            </div>
          </div>
        )}
          ...
          <div style={{ padding: '20px', borderBottom: '1px solid var(--border)', background: 'var(--bg-elevated)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr 1fr', gap: '10px', marginBottom: '12px' }}>
              ...
              <div>
                ...
                <div style={{ gridColumn: 'span 5', marginTop: '4px' }}>
                  <label style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'block', marginBottom: '8px' }}>
                    Fiyatlandırma Tipi
                  </label>
                  <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                    {[
                      ...
                      { key: 'quantity', label: 'Miktara Göre' },
                      { key: 'range', label: 'Range / Slider' },
                      { key: 'options', label: 'Seçenek Bazlı' },
                    ].map(t => (
                      <button key={t.key} type="button"
                        onClick={() => setServiceForm(f => ({ ...f, pricingType: t.key }))}
                        style={{
                          padding: '6px 14px', borderRadius: '20px', fontSize: '12px',
                          fontFamily: 'var(--font-montserrat)', fontWeight: '600',
                          cursor: 'pointer', border: '1px solid',
                          background: serviceForm.pricingType === t.key ? 'var(--gold)' : 'transparent',
                          color: serviceForm.pricingType === t.key ? '#0a0a0a' : 'var(--text-muted)',
                          borderColor: serviceForm.pricingType === t.key ? 'var(--gold)' : 'var(--border)',
                        }}>{t.label}</button>
                    ))}
                  </div>

                  {serviceForm.pricingType === 'quantity' && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '10px', padding: '14px', background: 'var(--bg)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                      <Field label="Birim Adı" placeholder="Divine Orb" value={serviceForm.pricingOptions.unitName}
                        onChange={v => setServiceForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, unitName: v } }))} />
                      <Field label="Birim Fiyatı (₺)" type="number" value={serviceForm.pricingOptions.unitPrice}
                        onChange={v => setServiceForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, unitPrice: v } }))} />
                      <Field label="Min Miktar" type="number" value={serviceForm.pricingOptions.minQty}
                        onChange={v => setServiceForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, minQty: v } }))} />
                      <Field label="Max Miktar" type="number" value={serviceForm.pricingOptions.maxQty}
                        onChange={v => setServiceForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, maxQty: v } }))} />
                    </div>
                  )}

                  {serviceForm.pricingType === 'range' && (
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '10px', padding: '14px', background: 'var(--bg)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                      <Field label="Birim Adı" placeholder="Level" value={serviceForm.pricingOptions.unitName}
                        onChange={v => setServiceForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, unitName: v } }))} />
                      <Field label="Birim Başına Fiyat (₺)" type="number" value={serviceForm.pricingOptions.pricePerUnit}
                        onChange={v => setServiceForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, pricePerUnit: v } }))} />
                      <Field label="Min Değer" type="number" value={serviceForm.pricingOptions.min}
                        onChange={v => setServiceForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, min: v } }))} />
                      <Field label="Max Değer" type="number" value={serviceForm.pricingOptions.max}
                        onChange={v => setServiceForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, max: v } }))} />
                    </div>
                  )}

                  {serviceForm.pricingType === 'options' && (
                    <div style={{ padding: '14px', background: 'var(--bg)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '10px' }}>
                        {(serviceForm.pricingOptions.choices || []).map((c, i) => (
                          <span key={i} style={{
                            display: 'flex', alignItems: 'center', gap: '6px',
                            background: 'var(--bg-elevated)', border: '1px solid var(--border)',
                            borderRadius: '8px', padding: '4px 10px', fontSize: '12px', color: '#fff',
                          }}>
                            {c.label} — {c.price}₺
                            <button type="button" onClick={() => setServiceForm(f => ({
                              ...f, pricingOptions: { ...f.pricingOptions, choices: f.pricingOptions.choices.filter((_, j) => j !== i) }
                            }))} style={{ background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', fontSize: '14px', padding: 0 }}>×</button>
                          </span>
                        ))}
                      </div>
                      <ChoiceAdder onAdd={choice => setServiceForm(f => ({
                        ...f, pricingOptions: { ...f.pricingOptions, choices: [...(f.pricingOptions.choices || []), choice] }
                      }))} />
                    </div>
                  )}
                </div>
              ...
              <div>
                ...
                <select value={serviceForm.serviceCategory} onChange={e => setServiceForm(f => ({ ...f, serviceCategory: e.target.value }))}
                  ...
                  {(game.serviceCategories || []).map((cat, i) => (
            ...
            <div style={{ marginBottom: '12px' }}>
              ...
              <textarea value={serviceForm.features} onChange={e => setServiceForm(f => ({ ...f, features: e.target.value }))}
                placeholder={'Tüm seviyelerde boost\nHesap güvenliği garantili\nVPN koruması'} rows={4}
                style={{ width: '100%', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '6px', padding: '8px 10px', color: '#fff', fontSize: '13px', fontFamily: 'var(--font-inter)', outline: 'none', resize: 'vertical' }} />
            </div>
            <div style={{ display: 'flex', gap: '8px' }}>
              <button className="btn-primary" style={{ fontSize: '13px', padding: '8px 16px' }} onClick={() => addService(game.id)}>Kaydet</button>
              <button className="btn-secondary" style={{ fontSize: '13px', padding: '8px 16px' }} onClick={() => setShowAddService(null)}>İptal</button>
            </div>
          </div>
        )}
              ...
              <thead>
                <tr>
                  {['Hizmet', 'Kategori', 'Fiyat', 'Tip', 'Durum', 'İşlemler'].map(h => (
                    <th key={h} style={{ textAlign: 'left', fontSize: '11px', color: 'var(--text-dim)', fontWeight: '600', fontFamily: 'var(--font-montserrat)', padding: '6px 8px', borderBottom: '1px solid var(--border)' }}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                  ...
                  <>
                    <tr key={s.id}>
                      <td style={{ padding: '10px 8px', fontSize: '13px', color: '#fff' }}>
                        {s.name}
                        {s.isHot && <span style={{ marginLeft: '6px', background: '#ff4444', color: '#fff', fontSize: '9px', fontWeight: '700', padding: '1px 5px', borderRadius: '3px', fontFamily: 'var(--font-montserrat)' }}>HOT</span>}
                      </td>
                      <td style={{ padding: '10px 8px', fontSize: '12px', color: 'var(--text-muted)' }}>{s.serviceCategory || 'Genel'}</td>
                      <td style={{ padding: '10px 8px', fontSize: '13px', color: 'var(--gold)', fontWeight: '600' }}>{s.basePrice.toLocaleString('tr-TR')} ₺</td>
                      <td style={{ padding: '10px 8px', fontSize: '12px', color: 'var(--text-muted)' }}>{s.priceType === 'fixed' ? 'Sabit' : 'Değişken'}</td>
                      <td style={{ padding: '10px 8px' }}>
                        <span style={{ fontSize: '11px', padding: '2px 8px', borderRadius: '20px', background: s.isActive ? '#1a2a1a' : '#2a1a1a', color: s.isActive ? '#4caf50' : '#ff6666', border: `1px solid ${s.isActive ? '#2a4a2a' : '#4a2a2a'}` }}>{s.isActive ? 'Aktif' : 'Pasif'}</span>
                      </td>
                      <td style={{ padding: '10px 8px' }}>
                        <div style={{ display: 'flex', gap: '6px' }}>
                          <button style={{ background: 'transparent', border: '1px solid var(--gold)', borderRadius: '6px', padding: '4px 10px', fontSize: '11px', color: 'var(--gold)', cursor: 'pointer' }}
                            onClick={() => {
                              ...
                              setEditForm({
                                ...
                                pricingType: s.options?.type || 'fixed',
                                pricingOptions: s.options ? {
                                  unitName: s.options.unitName || '',
                                  unitPrice: s.options.unitPrice || '',
                                  minQty: s.options.minQty || 1,
                                  maxQty: s.options.maxQty || 999,
                                  pricePerUnit: s.options.pricePerUnit || '',
                                  min: s.options.min || 1,
                                  max: s.options.max || 100,
                                  choices: s.options.choices || [],
                                } : {
                                  unitName: '', unitPrice: '', minQty: 1, maxQty: 999,
                                  pricePerUnit: '', min: 1, max: 100, choices: [],
                                }
                              })
                            }}>
                            {editService === s.id ? 'Kapat' : 'Düzenle'}
                          </button>
                          <button style={{ background: 'transparent', border: '1px solid var(--border)', borderRadius: '6px', padding: '4px 10px', fontSize: '11px', color: 'var(--text-muted)', cursor: 'pointer' }}
                    ...
                    {editService === s.id && (
                      <tr key={`edit-${s.id}`}>
                        <td colSpan={6} style={{ padding: '0 8px 12px' }}>
                          <div style={{ background: 'var(--bg-elevated)', borderRadius: '10px', border: '1px solid var(--gold)', padding: '16px', marginTop: '8px' }}>
                            <h4 style={{ color: 'var(--gold)', fontSize: '13px', marginBottom: '12px', fontFamily: 'var(--font-montserrat)', fontWeight: '600' }}>Düzenle — {s.name}</h4>
                                ...
                                <div style={{ gridColumn: 'span 4', marginTop: '4px' }}>
                                  ...
                                  <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
                                    {[
                                      ...
                                      <button key={t.key} type="button"
                                        onClick={() => setEditForm(f => ({ ...f, pricingType: t.key }))}
                                        style={{
                                          padding: '6px 14px', borderRadius: '20px', fontSize: '12px',
                                          fontFamily: 'var(--font-montserrat)', fontWeight: '600',
                                          cursor: 'pointer', border: '1px solid',
                                          background: editForm.pricingType === t.key ? 'var(--gold)' : 'transparent',
                                          color: editForm.pricingType === t.key ? '#0a0a0a' : 'var(--text-muted)',
                                          borderColor: editForm.pricingType === t.key ? 'var(--gold)' : 'var(--border)',
                                        }}>{t.label}</button>
                                    ))}
                                  </div>
                                  ...
                                  {editForm.pricingType === 'quantity' && (
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '10px', padding: '14px', background: 'var(--bg)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                                      <Field label="Birim Adı" placeholder="Divine Orb" value={editForm.pricingOptions?.unitName || ''}
                                        onChange={v => setEditForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, unitName: v } }))} />
                                      <Field label="Birim Fiyatı (₺)" type="number" value={editForm.pricingOptions?.unitPrice || ''}
                                        onChange={v => setEditForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, unitPrice: v } }))} />
                                      <Field label="Min Miktar" type="number" value={editForm.pricingOptions?.minQty || 1}
                                        onChange={v => setEditForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, minQty: v } }))} />
                                      <Field label="Max Miktar" type="number" value={editForm.pricingOptions?.maxQty || 999}
                                        onChange={v => setEditForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, maxQty: v } }))} />
                                    </div>
                                  )}

                                  {editForm.pricingType === 'range' && (
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '10px', padding: '14px', background: 'var(--bg)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                                      <Field label="Birim Adı" placeholder="Level" value={editForm.pricingOptions?.unitName || ''}
                                        onChange={v => setEditForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, unitName: v } }))} />
                                      <Field label="Birim Başına Fiyat (₺)" type="number" value={editForm.pricingOptions?.pricePerUnit || ''}
                                        onChange={v => setEditForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, pricePerUnit: v } }))} />
                                      <Field label="Min Değer" type="number" value={editForm.pricingOptions?.min || 1}
                                        onChange={v => setEditForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, min: v } }))} />
                                      <Field label="Max Değer" type="number" value={editForm.pricingOptions?.max || 100}
                                        onChange={v => setEditForm(f => ({ ...f, pricingOptions: { ...f.pricingOptions, max: v } }))} />
                                    </div>
                                  )}
                                  ...
                                  {editForm.pricingType === 'options' && (
                                    <div style={{ padding: '14px', background: 'var(--bg)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                                      ...
                                      <ChoiceAdder onAdd={choice => setEditForm(f => ({
                                        ...f, pricingOptions: { ...f.pricingOptions, choices: [...(f.pricingOptions?.choices || []), choice] }
                                      }))} />
                                    </div>
                                  )}
                            ...
                            <div style={{ marginBottom: '12px' }}>
                              <label style={{ fontSize: '12px', color: 'var(--text-muted)', display: 'block', marginBottom: '4px' }}>Özellikler (her satır ayrı madde)</label>
                              <textarea value={editForm.features} onChange={e => setEditForm(f => ({ ...f, features: e.target.value }))} rows={4}
                                style={{ width: '100%', background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: '6px', padding: '8px 10px', color: '#fff', fontSize: '13px', fontFamily: 'var(--font-inter)', outline: 'none', resize: 'vertical' }} />
                            </div>
                            <div style={{ display: 'flex', gap: '8px' }}>
                              <button className="btn-primary" style={{ fontSize: '13px', padding: '8px 16px' }} onClick={saveEditService}>Kaydet</button>
                              <button className="btn-secondary" style={{ fontSize: '13px', padding: '8px 16px' }} onClick={() => setEditService(null)}>İptal</button>
                            </div>
                          </div>
                        </td>
                      </tr>
...
function ChoiceAdder({ onAdd }) {
  ...
  return (
    <div style={{ display: 'flex', gap: '8px', alignItems: 'flex-end' }}>
      ...
      <button type="button" className="btn-secondary" style={{ fontSize: '12px', padding: '7px 14px' }}
        onClick={() => {
          if (label.trim() && price) {
            onAdd({ label: label.trim(), price: parseFloat(price) })
            setLabel('')
            setPrice('')
          }
        }}>Ekle</button>
